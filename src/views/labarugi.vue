<template>
  <div class="app-container" style="width:100%; box-shadow:2">
    <el-date-picker v-model="start" style="margin-left:20px; width:140px; margin-bottom:10px" width="140px" class="filter-item" type="date" placeholder="Dari" />
    <el-date-picker v-model="end" style="margin-left:8px;width:140px;" class="filter-item" type="date" placeholder="Sampai" />
    <el-button class="filter-item" style="margin-left: 20px;" type="primary" icon="el-icon-edit" @click="handleFilterByDate">
      Filter
    </el-button>

    <el-button v-print="'#printMe'" type="primary">Print</el-button>
    <div id="printMe">
      <b><hr></b>
      ...
      <b><hr></b>
      <el-tree ref="tree" :data="listHarta" default-expand-all node-key="id" highlight-current :props="defaultProps">
      ...
      <el-tree ref="tree" :data="listModal" default-expand-all node-key="id" highlight-current :props="defaultProps">
        <span slot-scope="{ node, data }" class="custom-tree-node">
          <span>{{ data.name }}</span>
          <span v-if="data.valueTotal != 0">{{ handleCurrency(data.valueTotal) }}</span>
          <span v-else>{{ handleCurrency(data.total) }}</span>
      ...
      <el-tree ref="tree" :data="listKewajiban" default-expand-all node-key="id" highlight-current :props="defaultProps">
        <span slot-scope="{ node, data }" class="custom-tree-node">
  ...
  methods: {
    getList() {
      ...
      axios.post('/report/Pendapatan').then((response) => {
        ...
        const names = response.data.akun[0]
