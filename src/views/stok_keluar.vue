  ...
  <div class="app-container">
      ...
      <el-table-column label="Actions" align="left" width="80" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-popover trigger="hover" placement="top">
            ...
            <el-button v-if="checkPermission(['admin'])" type="primary" size="mini" @click="handleDelete(row)">
              Delete
            </el-button>
            <br>
            <br>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="top" label-width="180px" style="width: 100% !important; margin-left:50px;" :inline="true">
        <el-form-item v-if="dialogStatus == 'create'" class="k" label="Customer">
        ...
        <div v-for="(all, index) in kasIn.all" v-if="dialogStatus == 'create'" style="display:flex; flex-wrap: wrap; width:100% !important">
          ...
          <el-form-item class="k" :label="index == 0 ? 'Harga Satuan' : ''">
            <v-money-spinner v-if="roles == 'admin'" v-model="all.harga" v-bind="config" required type="text" placeholder="Rp 0" @change="onChangeQty(index)" />
            <v-money-spinner v-else v-model="all.harga" v-bind="config" readonly required type="text" placeholder="Rp 0" @change="onChangeQty(index)" />
          </el-form-item>
          <el-form-item class="k" :label="index == 0 ? 'Sub Total':''">
            <v-money-spinner v-model="all.total" v-bind="config" disabled placeholder="Rp 0" @change="onChangeTotal()" />
          </el-form-item>
        ...
        <el-form-item class="k" label="Jumlah Pembayaran">
          <v-money-spinner v-model="jumlah_bayar" v-bind="config" placeholder="Rp 0" @change="handleChangeText()" />
        </el-form-item>
        <el-form-item v-if="dialogStatus == 'create'" class="k" label="Potongan">
          <v-money-spinner v-model="discount" placeholder="Rp 0" v-bind="config" @change="handleChangeText()" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer" style="display:flex; flex-wrap:wrap; justify-content:center">
        <el-button v-if="dialogStatus == 'create'" style="margin:20px 10px" type="primary" @click="addFind">
          Tambah Produk
        </el-button>
  ...
  methods: {
    ...
    createData() {
      ...
      const data = {
        ...
        cashin_id: this.cashout_id == '' ? this.kas[0].id : this.cashout_id,
        product_id,
        qty,
        selling_price,
        date: this.dates,
